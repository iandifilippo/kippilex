...
// --- Constantes de datos (no modificadas) ---
...
// --- Componente Modal (no modificado) ---
...
export default function LawyerRegistrationForm() {
    ...
    // Almacena un array de keys de categorías abiertas para mejorar la usabilidad con el buscador
    const [openCategories, setOpenCategories] = useState<string[]>([]);

    useEffect(() => {
        // ... (Lógica para precargar nombre y apellido desde Supabase)
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (type === 'file') {
            // Manejo optimizado para files, asegurando que se guarda el archivo o null si se borra
    ...
    const handleCheckboxChange = (subspecName: string) => {
        ...
        if (isSelected) {
            ...
            // Límite opcional de especialidades (ej: 10)
            if (formData.especialidades.length >= 10) {
                 setFormError('Máximo 10 especialidades permitidas.');
                 return;
            }
    ...
    // Función para alternar la visibilidad de la categoría
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Nueva Validación de Tarjeta Profesional: Alfanumérico, guiones, y máximo 20 caracteres
        if (!/^[a-zA-Z0-9\s-]{1,20}$/.test(formData.tarjetaProfesional)) {
            setFormError('El Número de Tarjeta Profesional no es válido (solo números, letras o guiones).');
            setLoading(false); return;
        }
        ...
        if (file.type !== 'application/pdf') {
             setFormError('El archivo subido debe ser un PDF.');
             setFormData(prev => ({ ...prev, documento: null })); // Limpiar archivo inválido
             setLoading(false); return;
        }
        ...
        if (userError || !user) {
            setFormError("Error crítico de sesión.");
            setLoading(false);
            return;
        }
...
if (insertDocError) {
  ...
  return;
    ...
    const filteredData = useMemo(() => {
        let newOpenCategories: string[] = [];
        
        if (!searchTerm) {
            // Si no hay búsqueda, solo se usa el estado de categorías abiertas manualmente
            return LEGAL_SPECIALTIES_DATA;
        }

        const lowerCaseSearch = searchTerm.toLowerCase();

        const data = LEGAL_SPECIALTIES_DATA.map(category => {
            const subs = category.subs.filter(sub => 
                sub.toLowerCase().includes(lowerCaseSearch)
            );
            
            // Si hay subespecialidades coincidentes, añadir la categoría para que se abra
            if (subs.length > 0) {
                newOpenCategories.push(category.key);
            }

            return { ...category, subs };
        }).filter(category => category.subs.length > 0);
        
        // Abrir automáticamente las categorías que contienen el resultado
                ...
                <div className="py-12 md:py-20">
                    <div className="pb-12 text-center">
                        <h1 
                            className="
                                animate-gradient 
                                bg-[linear-gradient(to_right,var(--color-indigo-200),var(--color-gray-50),var(--color-indigo-300))] 
                                bg-size-[200%_auto] 
                                bg-clip-text 
                                pb-5 
                                font-nacelle 
                    ...
                    <form onSubmit={handleSubmit} className="relative rounded-2xl bg-gray-900/50 p-8 shadow-2xl backdrop-blur-sm">
                        ...
                        {/* SECCIÓN 2: INFORMACIÓN PROFESIONAL Y ESPECIALIDADES */}
                        <div className="flex items-center gap-2 mb-6">
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <input id="tarjetaProfesional" type="text" name="tarjetaProfesional" value={formData.tarjetaProfesional} onChange={handleInputChange} className="form-input w-full" placeholder="Ej: 123456 o T-12345" required />
                            </div>
                        </div>

                        <h3 className="mt-8 mb-4 text-lg font-medium text-gray-200">Especialidades Legales</h3>
                        <p className="mb-6 text-sm text-indigo-200/65">Las notificaciones de nuevos casos se enviarán de acuerdo a las especialidades que selecciones. **Máximo 10 especialidades**.</p>
                        ...
                        <div className="space-y-3">
                            {filteredData.map((category) => (
                                <div key={category.key} className="rounded-lg border border-gray-800 bg-gray-800/40">
                                    ...
                                    >
                                        ...
                                        <svg className={`h-5 w-5 transition-transform ${openCategories.includes(category.key) ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                                    </button>
                                            ...
                                            {category.subs.map((subspec) => (
                                                <div key={subspec} className="flex items-center">
                                                    <input
                                                        ...
                                                        // Deshabilitar si se alcanza el límite y no está seleccionada
                                                        disabled={formData.especialidades.length >= 10 && !formData.especialidades.includes(subspec)}
                                                    />
                                                    <label htmlFor={subspec} className={`ml-2 text-sm ${formData.especialidades.length >= 10 && !formData.especialidades.includes(subspec) ? 'text-gray-500' : 'text-indigo-200/65'}`}>{subspec}</label>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                            ...
                            <div className="pt-4 text-xs text-indigo-200/65">
                                Seleccionadas: **{formData.especialidades.length}** de 10. ({formData.especialidades.join(', ') || 'Ninguna'})
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <label className="text-sm font-medium text-indigo-200/65" htmlFor="documentUpload">
                                Subir documentos (PDF ÚNICAMENTE)
                            </label>
                            <p className="text-xs text-red-400">Debe subir UN SOLO archivo PDF que contenga los 3 elementos solicitados:</p>
                            ...
                            />
                            {formData.documento && (
                                <p className="text-xs text-green-400">Archivo seleccionado: {formData.documento.name}</p>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-4">
                            ...
                            {formError && (
                                <p className="mt-4 text-center text-red-400 text-sm font-medium p-2 bg-red-900/20 rounded-lg">{formError}</p>
                            )}
